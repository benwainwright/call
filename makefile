...
update-tap: releases/$(RELEASE_VERSION).tar.gz
	...
	sed -E -i .bak 's/(https:\/\/github\.com\/benwainwright\/call\/archive\/)[0-9]+\.[0-9]+\.[0-9]+\.tar\.gz/\1$(RELEASE_VERSION)\.tar.gz/g' .tap-clone/Formula/pycall.rb
	sed -E -i .bak "s/(sha256 )'[0-9a-z]*'/\1'$(SHASUM)'/g" .tap-clone/Formula/pycall.rb
	cd .tap-clone && git add Formula/pycall.rb && git commit -m "Update $(NAME) to $(RELEASE_VERSION)" && git push origin master
	rm -rf .tap-clone
