...
def test_correctly_constructs_url_without_options():
    ...
    endpoint = Endpoint(
        ...
        paths={"foo": Path(method="get", name="foo", route="foo/bar", options=[])},
    )
...
def test_correctly_constructs_url_with_single_arg():
    ...
    endpoint = Endpoint(
        ...
        paths={
            "foo": Path(
                method="get", name="foo", route="foo/bar/{}/bar", options=["foo"]
            )
        },
    )

    args = [pyargs.Argument(name="foo", description="blah", required=True, value="baz")]
...
def test_correctly_constructs_url_with_query_that_has_constant_parts():
    ...
    args = [
        pyargs.Argument(name="fire", description="blah", required=True, value="bright"),
        pyargs.Argument(name="fish", description="blah", required=True, value="wet"),
        pyargs.Argument(name="foo", description="blah", required=True, value="bar"),
    ]
    ...
    assert (
        request.url
        == "https://www.google.com/foo/bar/bar/wet?bar=baz&fire=bright&blob=blah"
    )
    assert request.method == "get"
...
def test_correctly_constructs_url_with_multiple_args():
    ...
    endpoint = Endpoint(
        ...
        paths={
            "foo": Path(
                name="foo", method="get", options=["foo", "fish"], route="foo/bar/{}/{}"
            )
        },
    )
